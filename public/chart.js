...
//
function querystring() {
    console.log('qs ', location, location.search)
    var q = location.search.substring(1).split('&');

    console.log('qs2 ', q, q[0],q[1]);
    return q[0];
...
//          console.log('parts ',parts, params[parts[1]])
//      return params;
//    }, {});
}
...
d3.json('json/codeps/' + packageName + '.json', function(error, display) {
  ['dependencies', 'devDependencies'].forEach(function (type) {
    display[type].name = packageName;
      console.log('name ',display[type].name)
